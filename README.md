...
$ cd script && python3 milvus_password.py
```
...
```shell
$ cd data_process && python3 parse_md.py
$ cd data_process && python3 build_index.py
```
...
```shell
$ cd rag_service && mv example_config.py config.py
$ vim config.py
```
...
```shell
$ cd rag_service && python3 rag_server.py
```
...
```shell
$ cd rag_chat && npm install
$ cd rag_chat && npm run serve
```
