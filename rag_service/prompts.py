...
# 用于上下文重写问题的Prompt
CONTEXT_REWRITE_PROMPT_TEMPLATE = """
你是一名擅长理解和重写用户问题的语言助手。你的任务是根据上下文对用户最新提出的问题进行重写，满足以下要求：

1. 确定上下文中哪些内容与用户最新的问题相关，只保留与当前问题直接相关的上下文信息，其余无关内容必须丢弃。
2. 调整用户最新问题中的指代信息（例如“它”、“这个”、“他们”等），用明确的表述替换，使问题即使脱离上下文也能够完整、清晰地表达用户的真实意图。
